...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CAsdf::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function base_url($url=null) {
  return CAsdf::Instance()->request->base_url . trim($url, '/');
}
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CAsdf::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function render_views($region='default') {
  return CAsdf::Instance()->views->Render($region);
}
