...
function autoload($aClassName) {
  $classFile = "/src/{$aClassName}/{$aClassName}.php";
	$file1 = ASDF_SITE_PATH . $classFile;
	$file2 = ASDF_INSTALL_PATH . $classFile;
	if(is_file($file1)) {
		require_once($file1);
...
function exceptionHandler($e) {
  echo "Asdf: Uncaught exception: <p>" . $e->getMessage() . "</p><pre>" . $e->getTraceAsString(), "</pre>";
}
...
function htmlEnt($str, $flags = ENT_COMPAT) {
  return htmlentities($str, $flags, CAsdf::Instance()->config['character_encoding']);
}
