...
class CAsdf implements ISingleton {
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= ASDF_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = ASDF_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $asf->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $asf
    ...
    $asf = &$this;
    // First the default Asdf themes/functions.php
  ...
  /**
   * Draw HTML for a menu defined in $asf->config['menus'].
   *
